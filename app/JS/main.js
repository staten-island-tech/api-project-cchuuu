...
function stats(data) {
    DOMselectors.container.addEventListener("click", (event) => {
      if (event.target.classList.contains("statsBtn")) {
        ...
        const selectedWeapon = data.data.find((weapon) => weapon.uuid === weaponId);
          ...
          DOMselectors.container.insertAdjacentHTML(
            ...
            `
              <div class="weapon-stats bg-red-500 p-4 rounded-lg mt-20 border-4 border-black">
                <h2 class="text-2xl font-bold">${selectedWeapon.displayName}</h2>
                <img src="${selectedWeapon.displayIcon}" alt="${selectedWeapon.displayName}" class="object-contain w-full h-48 rounded-lg shadow-md"/>
                <p class="mt-2">Category: ${selectedWeapon.shopData?.category || "N/A"}</p>
                <p>Cost: $${selectedWeapon.shopData?.cost || "N/A"}</p>
                <p>Fire Rate: ${selectedWeapon.weaponStats?.fireRate || "N/A"}</p>
                <p>Magazine Size: ${selectedWeapon.weaponStats?.magazineSize || "N/A"}</p>
                <p>Reload Time: ${selectedWeapon.weaponStats?.reloadTimeSeconds || "N/A"}s</p>
